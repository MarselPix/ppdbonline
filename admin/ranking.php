...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['hitung_ranking'])) {
    ...
    // Jalur Prestasi Non-Akademik: Placeholder. Seharusnya ada sistem skor.
    // Untuk saat ini, kita urutkan berdasarkan nama sebagai contoh.
...
foreach ($jalur_types as $jalur) {
    $query = "SELECT no_pendaftaran, nama_lengkap, rata_rata_nilai_rapot, ranking FROM calon_siswa WHERE jalur_pendaftaran = ? AND ranking IS NOT NULL ORDER BY ranking ASC";
            ...
            <tbody>
                <?php if(!empty($ranked_lists[$jalur])): ?>
                    ...
                    <tr>
                        ...
                        <td><?php echo $siswa['rata_rata_nilai_rapot'] ?? 'N/A'; ?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
